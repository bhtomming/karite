...
<div class="content">
    ...
    <table class="tb">
        ...
        <tr>
            <td>磁盘空间</td>
            <td>10M+</td>
            <td><?php echo function_exists('disk_free_space') ? get_byte(disk_free_space(DPCMS_ROOT)) : 'unknow'; ?></td>
        </tr>
        <tr>
            <td>mysql扩展</td>
            <td>必须开启</td>
    ...
    <table class="tb">
        ...
        $dirs = array('admin', 'log', APP_NAME.'/install', 'module', 'upload');
        foreach($dirs as $dir) {
            $ret = _dir_write(DPCMS_ROOT.'/'.$dir, TRUE);
            ...
            if(!empty($ret['no'])) {
                ...
                foreach($ret['no'] as $i => $row) {
                    echo '<br>['.$row[1].'] '.str_replace(DPCMS_ROOT, '', $row[0]);
                    if($i>8) {
                        echo '<br>******'; break;
                    }
                }
...
<div class="button">
    <?php if($err) { ?>
        <a href="javascript:;" class="grey">下一步</a><a href="index.php?step=reade">上一步</a>
    <?php }else{ ?>
        <a href="index.php?step=check_db">下一步</a><a href="?step=reade">上一步</a>
    <?php } ?>
</div>
